...
import { DAYS_IN_MONTH, SUBSIDY_INCOME_CAP, SUBSIDY_UMA_MULTIPLIER_2026, SUBSIDY_UMA_MULTIPLIER_JAN_2026, CESANTIA_VEJEZ_TABLE, IMSS_RATES } from '../constants';
...
export const DataTablesPanel: React.FC<DataTablesPanelProps> = ({ isrTable, cyvTable, uma, calculationDate }) => {
        const [activeTab, setActiveTab] = useState<'isr' | 'subsidy' | 'cyv' | 'imss'>('isr');
    ...
    const renderSubsidyInfo = () => {
        ...
        const renderImssTable = () => {
        const rateNames: { [key: string]: string } = {
            FIXED_QUOTA_PCT: 'Cuota Fija',
            EXCESS_QUOTA_PCT: 'Excedente (3 UMA)',
            CASH_BENEFITS_PCT: 'Prestaciones en Dinero',
            MEDICAL_PENSIONERS_PCT: 'Gastos Médicos Pensionados',
            DISABILITY_LIFE_PCT: 'Invalidez y Vida',
            RETIREMENT_PCT: 'Retiro',
            NURSERY_PCT: 'Guarderías',
            CESANTIA_VEJEZ_PCT: 'Cesantía y Vejez',
        };

        const rateNotes: { [key: string]: string } = {
            FIXED_QUOTA_PCT: 'Sobre 1 UMA',
            EXCESS_QUOTA_PCT: 'Sobre diferencia (SBC - 3 UMA)',
        };

        const renderTableSection = (title: string, rates: { [key: string]: number }) => (
            <div className="mb-6">
                    ...
                    <table className="min-w-full text-sm text-left text-slate-600">
                        <thead className="text-xs text-slate-700 uppercase bg-slate-100">
                            <tr>
                                ...
                                <th scope="col" className="px-4 py-2 text-right">Tasa</th>
                                <th scope="col" className="px-4 py-2">Nota</th>
                            </tr>
                        </thead>
                        <tbody>
                            {Object.entries(rates).map(([key, value]) => (
                                <tr key={key} className="bg-white border-b border-slate-200 hover:bg-slate-50">
                                    <td className="px-4 py-2">{rateNames[key] || key}</td>
                                    <td className="px-4 py-2 text-right">{value.toFixed(4)}%</td>
                                    <td className="px-4 py-2 text-xs text-slate-500">{rateNotes[key] || ''}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
        ...
        return (
            <div className="p-4">
                {renderTableSection("Cuotas Patrón", IMSS_RATES.EMPLOYER)}
                {renderTableSection("Cuotas Obrero", IMSS_RATES.EMPLOYEE)}
            </div>
        );
    };
    ...
    const renderCyVTable = () => (
        ...
        const getTabClass = (tabName: 'isr' | 'subsidy' | 'cyv' | 'imss') => {
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
            ...
            <div className="mb-4 flex space-x-2 border-b border-slate-200 pb-2">
                ...
                <button onClick={() => setActiveTab('imss')} className={getTabClass('imss')}>IMSS</button>
            </div>
            <div className="max-h-64 overflow-y-auto">
                {activeTab === 'isr' && renderIsrTable()}
                {activeTab === 'subsidy' && renderSubsidyInfo()}
                                {activeTab === 'cyv' && renderCyVTable()}
                {activeTab === 'imss' && renderImssTable()}
            </div>
        </div>
    );
};