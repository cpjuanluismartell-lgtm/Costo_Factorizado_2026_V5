...
export const ResultsTable: React.FC<ResultsTableProps> = ({ results, uma, workRisk, isn, layout, percepcionHeaders, onSchemeChange, desiredNetSalaries, onDesiredNetChange }) => {
    ...
    const staticHeaders = [
        ...
        "Exc. Obr",
        "Prest. en Dinero Obr",
        "Gastos Médicos Pens Obr",
        "Invalidez y Vida Obr",
        "IMSS Obrero",
        "C y V Obr",
        "Total Obrero",
        "Total Percepciones",
                ...
                <table className="min-w-full divide-y divide-slate-200 border-separate border-spacing-0">
                    ...
                    <tbody className="bg-white">
                        {results.map((res, index) => {
                            ...
                            const sdiTooltip = (
                                <div className="text-left text-xs space-y-1">
                                    ...
                                    <div className="pt-1">
                                        ...
                                        {details.variablePerceptions.length > 0 && (
                                            <div className="mt-1 pl-2 border-l border-slate-600 space-y-0.5">
                                                {details.variablePerceptions.map((p, idx) => (
                            ...
                            const isnTooltip = (
                                <div className="text-left text-xs space-y-1 min-w-[240px]">
                                    <h4 className="font-bold border-b border-slate-600 pb-1 mb-2">Base Impuesto Sobre Nómina</h4>
                                    <div className="space-y-1">
                                        <div className="flex justify-between">
                                            <span>Sueldo Base:</span>
                                            <span>{formatCurrency(res.sueldoMensualBase)}</span>
                                        </div>
                                        {res.percepcionesDetalladas
                                            .filter(p => !['sueldo', 'salario', 'sueldo mensual', 'sueldo base', 'sueldo ordinario', 'vacaciones a tiempo'].includes(p.name.toLowerCase()))
                                            .map((p, idx) => {
                                                const isExempt = ['indemnización', 'indemnizacion', 'liquidación', 'liquidacion', 'finiquito'].some(en => p.name.toLowerCase().includes(en));
                                                return (
                                                    <div key={idx} className="flex justify-between">
                                                        <span className={`truncate pr-2 ${isExempt ? 'text-slate-400' : ''}`}>{p.name}:</span>
                                                        <span className={isExempt ? 'text-slate-400 line-through' : ''}>{formatCurrency(p.value)}</span>
                                                    </div>
                                                );
                                            })
                                        }
                                        <div className="flex justify-between">
                                            <span>Prov. Aguinaldo:</span>
                                            <span>{formatCurrency(res.aguinaldoMensual)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Prov. Prima Vac.:</span>
                                            <span>{formatCurrency(res.primaVacacionalMensual)}</span>
                                        </div>
                                        {details.isnExemptValue > 0 && (
                                            <div className="flex justify-between text-emerald-400">
                                                <span>(-) Conceptos Exentos:</span>
                                                <span>-{formatCurrency(details.isnExemptValue)}</span>
                                            </div>
                                        )}
                                        <div className="border-t border-slate-600 mt-1 pt-1 flex justify-between font-bold">
                                            <span>Base Total:</span>
                                            <span>{formatCurrency(details.isnBase)}</span>
                                        </div>
                                        <div className="flex justify-between text-sky-300 pt-1">
                                            <span>Tasa ISN ({isn.toFixed(2)}%):</span>
                                            <span>{formatCurrency(res.isn)}</span>
                                        </div>
                                    </div>
                                </div>
                            );
                                    ...
                                    <div className="grid grid-cols-2 gap-x-2 pt-1">
                                        ...
                                        <div className="col-span-2 border-t border-slate-600 my-1 pt-2">
                                            ...
                                            <div className="grid grid-cols-2 gap-x-2">
                                                {!res.percepcionesDetalladas.some(p => ['sueldo', 'salario', 'sueldo mensual', 'sueldo base', 'sueldo ordinario'].includes(p.name.toLowerCase())) && (
                                                    <React.Fragment>
                            ...
                            const taxableIncomeTooltip = (
                                <div className="text-left text-xs space-y-1 min-w-[280px]">
                                    <h4 className="font-bold border-b border-slate-600 pb-1 mb-2">Integración del Ingreso Gravable</h4>
                                    <div className="grid grid-cols-4 gap-x-2 pt-1 font-bold text-[10px] uppercase text-slate-400">
                                        <div className="col-span-1">Concepto</div>
                                        <div className="text-right">Monto</div>
                                        <div className="text-right">Exento</div>
                                        <div className="text-right">Gravado</div>
                                    </div>
                                    
                                    <div className="grid grid-cols-4 gap-x-2 py-1 border-b border-slate-700">
                                        <div className="col-span-1 truncate">Sueldo Base</div>
                                        <div className="text-right">{formatCurrency(res.sueldoMensualBase)}</div>
                                        <div className="text-right">{formatCurrency(0)}</div>
                                        <div className="text-right">{formatCurrency(res.sueldoMensualBase)}</div>
                                    </div>

                                    {details.percepcionesDetalleCalculo.map((p, index) => (
                                        <div key={index} className="grid grid-cols-4 gap-x-2 py-0.5">
                                            <div className="col-span-1 truncate" title={p.name}>{p.name}</div>
                                            <div className="text-right">{formatCurrency(p.value)}</div>
                                            <div className="text-right text-emerald-400">{p.exempt > 0 ? `-${formatCurrency(p.exempt)}` : formatCurrency(0)}</div>
                                            <div className="text-right">{formatCurrency(p.taxable)}</div>
                                        </div>
                                    ))}
                                    
                                    <div className="grid grid-cols-4 gap-x-2 border-t border-slate-600 mt-2 pt-2 font-bold">
                                        <div className="col-span-1">TOTALES</div>
                                        <div className="text-right">{formatCurrency(res.totalPercepciones)}</div>
                                        <div className="text-right text-emerald-400">-{formatCurrency(res.totalPercepciones - details.taxableIncome)}</div>
                                        <div className="text-right text-sky-300">{formatCurrency(details.taxableIncome)}</div>
                                    </div>
                                </div>
                            );
                            ...
                            const commonCells = (
                                <>
                                    ...
                                    {/* Detailed Calculation Columns */}
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">
                                        <InteractiveCell tooltipContent={taxableIncomeTooltip} value={formatCurrency(details.taxableIncome)} position={tooltipPosition} />
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.isrLowerLimit)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.isrExcess)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{details.isrPercentOverExcess.toFixed(2)}%</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.isrMarginalTax)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.isrFixedQuota)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.isrBruto)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.subsidioEmpleo)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200 font-bold">{formatCurrency(details.isr)}</td>
                                    
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.imssObreroDetalle.excedente)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.imssObreroDetalle.prestacionesDinero)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.imssObreroDetalle.gastosMedicos)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.imssObreroDetalle.invalidezVida)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200 font-bold">{formatCurrency(details.imssObrero)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200">{formatCurrency(details.imssObreroDetalle.cesantiaVejez)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 align-top border-b border-slate-200 font-bold">{formatCurrency(details.imssObrero)}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600 align-top border-b border-slate-200">{formatCurrency(details.isr)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600 align-top border-b border-slate-200">{formatCurrency(details.imssObrero)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-emerald-700 font-bold align-top border-b border-slate-200">{formatCurrency(res.salarioNeto)}</td>
                                </>
                            );
