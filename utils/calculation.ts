...
const performCalculation = ({
    ...
    const salaryLikeNames = ['sueldo', 'salario', 'sueldo mensual', 'sueldo base', 'sueldo ordinario'];
    ...
    const salarioDiario = sueldoMensualBaseOverride !== undefined 
        ? sueldoMensualBaseOverride / 30 
        : originalSalarioDiario;

    const sueldoMensualBase = sueldoMensualBaseOverride ?? (hasSalaryColumn ? salaryColumnValue : salarioDiario * 30);

    if (isNaN(sueldoMensualBase) || isNaN(salarioDiario) || sueldoMensualBase < 0) return null;
    ...
    const nonVariablePerceptionNames = new Set(['seguro gmm', 'seguro de vida', 'seguro vida', 'despensa', 'horas extras', 'pasivo laboral', 'vacaciones a tiempo', ...salaryLikeNames]);
    ...
    const variablePerceptions = percepcionesDetalladas.filter(p => !nonVariablePerceptionNames.has(p.name.toLowerCase()));
    ...
    const isnExemptNames = ['indemnización', 'indemnizacion', 'liquidación', 'liquidacion', 'finiquito'];
    const isnExemptValue = percepcionesDetalladas.reduce((acc, p) => {
        const name = p.name.toLowerCase();
        if (isnExemptNames.some(en => name.includes(en))) return acc + p.value;
        return acc;
    }, 0);

    const isnBase = Math.max(0, totalPercepciones + aguinaldoMensual + primaVacacionalMensual - isnExemptValue);
    const isnCosto = isnBase * (isn / 100);
    ...
    const percepcionesDetalleCalculo: { name: string; value: number; taxable: number; exempt: number }[] = [];
    let taxableIncome = sueldoMensualBase;

    percepcionesDetalladas.forEach(p => {
        const name = p.name.toLowerCase();
        // Skip salary-like names if they are already in sueldoMensualBase
        if (salaryLikeNames.includes(name)) return;

        let exempt = 0;
        if (name.includes('herramientas')) {
            exempt = p.value;
        } else if (name.includes('gmm') || name.includes('gastos medicos') || name.includes('seguro de vida') || name.includes('seguro vida')) {
            exempt = p.value;
        } else if (name.includes('despensa')) {
            if (!name.includes('efectivo')) {
                // Exenta hasta el 40% de la UMA diaria (art. 93 fr. XIV LISR)
                // Se asume mensualizado con factor 30.4
                exempt = Math.min(p.value, 0.4 * uma * 30.4);
            }
        }
        
        const taxable = Math.max(0, p.value - exempt);
        taxableIncome += taxable;
        percepcionesDetalleCalculo.push({ name: p.name, value: p.value, taxable, exempt });
    });
    ...
    let isrLowerLimit = 0;
    let isrExcess = 0;
    let isrPercentOverExcess = 0;
    let isrFixedQuota = 0;
    let isrMarginalTax = 0;

    if (isrBracket) {
        isrLowerLimit = isrBracket.lowerLimit;
        isrExcess = taxableIncome - isrLowerLimit;
        isrPercentOverExcess = isrBracket.percentOverExcess;
        isrFixedQuota = isrBracket.fixedQuota;
        isrMarginalTax = isrExcess * (isrPercentOverExcess / 100);
        isrBruto = isrMarginalTax + isrFixedQuota;
    }
